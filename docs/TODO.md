...
- [x] - [x] Analog I/O now crosses the scan boundary — `TagType.ANALOG_INPUT` / `ANALOG_OUTPUT` added; the input image freezes `DISCRETE_INPUT` + `ANALOG_INPUT`, `Executive._transfer_outputs` ships `DISCRETE_OUTPUT` + `ANALOG_OUTPUT`. `WORD` no longer claims `%IW`/`%QW` addresses — only the new analog types can. `demo.py`'s `tank_level` moved off `%MW0` onto `%IW0.0`; `start_stop_tank.py` reads it via `read_input`

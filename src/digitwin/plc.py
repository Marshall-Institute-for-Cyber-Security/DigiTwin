...
class PLC(ABC):
    ...
    def tag_at(self, native_address: str) -> str | None:
        ...
        naming the tag directly. Raises AddressError if ``native_address``
        isn't valid syntax for this model. A transport wired to a terminal
        with no tag claiming it should treat that as a config error, not
        silently drop the value — see InProcessTransport / ModbusSlaveServer.
        """

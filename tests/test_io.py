...
terminal-based wiring (digitwin/io.py) and PLC.tag_name_at (digitwin/plc.py).
...
def test_tag_name_at_resolves_a_defined_terminal() -> None:
    plc = _plc()
    plc.define_tag("start_button", TagType.DISCRETE_INPUT, False, "%I0.0")

    assert plc.tag_name_at("%I0.0") == "start_button"


def test_tag_name_at_returns_none_for_an_unwired_terminal() -> None:
    plc = _plc()

    assert plc.tag_name_at("%I0.5") is None


def test_tag_name_at_raises_on_malformed_syntax() -> None:
    plc = _plc()
    with pytest.raises(AddressError):
        plc.tag_name_at("%QX0.0")
...
def test_transport_resolves_the_tag_currently_at_a_terminal() -> None:
    ...
    transport = InProcessTransport(plc, bus, inputs={"%IW0.0": "tank_level"})
...
def test_transport_writes_the_output_tag_at_the_wired_terminal() -> None:
    ...
    bus = IOBus()
    transport = InProcessTransport(plc, bus, outputs={"%Q0.0": "fill_valve_signal"})
